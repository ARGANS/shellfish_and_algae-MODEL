...
\begin{equation}
    \displaystyle
    CR = 0.320+(0.3233\cdot{}T) – (0.0113\cdot{}T^2)
\end{equation}
```

Where T is the temperature in $\deg$C. The resulting functions are not particularly different in shape and it is highly preferable to have the same form of equation parameterised to respresent different species for simplest possible model implementation. Therefore we derive a new fit for *C. Gigas* using the form of the *M. edulis* equation. 
...
$\NIRREMORG$ is the net ingestion rate of $\REMORG$ and again is preseted in 2 ways by @Hawkins2013. Firstly, experimetal data for both species is used to fit an equation of the form:
...
The number of spawning events per year ($\NSE$) is limited to 2. In the `desolve` implementation of the model in R spawning is an event which only occurs when the above conditions have been met and not more than one spawning event has occurred within the previous 365 days. 
