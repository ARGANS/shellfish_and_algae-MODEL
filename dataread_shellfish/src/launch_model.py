...
class SF_model_scipy:
    ...
    def derivative(y, data, self):
        ...
        NIRSELORG = (p.m_NSO*SELORG)*TEF*(DSTW**0.62)*(data['CHL_ext'] > 0.01) + 0*(data['CHL_ext'] <= 0.01) #mg/h
        NIRREMORG = p.a_NRO*(1- np.exp(p.b_NRO*REMORG))*TEF*(DSTW**0.62) #mg/h
        NEA = p.Assimilation_efficiency*24*(NIRSELORG*p.ESELORG + NIRREMORG*p.Bioavailable_detrital_fraction*EREM) 
        MHL = 4.005*24*(np.exp(data['SST']*p.a_TEM)/np.exp(15*p.a_TEM))*DSTW**(0.72) #J/day
