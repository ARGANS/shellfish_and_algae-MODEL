...
\begin{equation}
    \displaystyle
    \Veff = \yfarm.(\xfarm+\flow).(\zfarm+\tz) (\#eq:Veff)
\end{equation}
```
...
\begin{equation}
    \displaystyle
    \frac{d\mathrm{NH_4}}{dt}=\min(1,\frac{\flow}{\xfarm}).(\ammonext-\ammon)+(\rlD  + \dmNf - f(\ammon,Q).B - \rnN).\frac{\VMA}{\Veff} (\#eq:NH4dt)
\end{equation}
```
...
\begin{equation}
    \displaystyle 
    \frac{d\mathrm{NO_3}}{dt}=\min(1,\frac{\flow}{\xfarm}).(\nitrext-\nitr)+(\rnN - f(\nitr,Q).B).\frac{\VMA}{\Veff} (\#eq:NO3dt)
\end{equation}
```
...
\begin{equation}
    \displaystyle 
    \frac{d\D}{dt}=\min(1,\frac{\flow}{\xfarm}).(\D_{ext}-\D)+ (\dmNf- \rlD).\frac{\VMA}{\Veff} (\#eq:D)
\end{equation}
```
...
```{r farmoparams, echo=FALSE}
  ...
  farmoparams<-data.frame(
    parameter=names(unlist(allparams$algae$farm_parameters)), 
    description=as.character(lapply(allparams$algae$farm_parameters_description, `[[`, 1)),
    units=as.character(lapply(allparams$algae$farm_parameters_description, `[[`, 2))
  )
