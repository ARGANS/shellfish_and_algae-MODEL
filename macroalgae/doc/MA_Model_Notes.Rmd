...
\begin{equation}
    \displaystyle
    \Veff = \yfarm\cdot(\xfarm+\flow\cdot{}dt)\cdot(\tz) (\#eq:Veff)
\end{equation}
```

The effect of horizontal advection is addressed separately from vertical mixing in the model (see Equations \@ref(eq:NH4dt) and \@ref(eq:NO3dt) for advection term), so it is further necessary to define a second characteristic volume, which is the volume of water that the farm is interacting with due to vertical mixing, neglecting advection, $\Vint$.


```{=tex}
\begin{equation}
    \displaystyle
    \Vint = \yfarm\cdot\xfarm\cdot\tz (\#eq:Vint)
\end{equation}
```



The ratio $\frac{\VMA}{\Vint}$ is used to scale the nutrient uptake within the farm to the net effect on concentration within the interacting volume. The further effect of 

We assume that the water column is well mixed to depth $\tz$ in water advected into the farm areas as well as within the farm itsself.  As nutrient flow will tend to restore nutrient concentration back to the ambient concentration, the ratio of advective flow  to farm length in the direction of flow  ($\frac{\flow\cdot{}dt}{\xfarm}$) can be used to determine the nutrient concentration restoration to the farm volume.
...
\begin{equation}
    \displaystyle
    \frac{d\mathrm{NH_4}}{dt}=\frac{\flow\cdot{}dt}{\xfarm}\cdot(\ammonext-\ammon)+(\rlD  + \dmNs - f(\ammon,Q)\cdot\B - \rnN)\cdot\frac{\VMA}{\Vint} (\#eq:NH4dt)
\end{equation}
```
...
\begin{equation}
    \displaystyle 
    \frac{d\mathrm{NO_3}}{dt}=\frac{\flow\cdot{}dt}{\xfarm}\cdot(\nitrext-\nitr)+(\rnN - f(\nitr,Q)\cdot\B)\cdot\frac{\VMA}{\Vint} (\#eq:NO3dt)
\end{equation}
```

In the above equations, B is the dry weight macroalgal biomass in grams per cubic meter, calculated as $\B=\frac{\Nf}{\Qmin}$. The pool of internal nitrogen stored by the macroalgae, $\Ns$ is controlled by the uptake of $\ammon$ and $\nitr$ into the store, the light, temperature and nutrient dependent growth rate (i.e. the uptake from stored inorganic N to fixed biomass N ($\Nf$)) and the mortality term. 
...
\begin{equation}
    \displaystyle 
    \frac{d\mathrm{N_f}}{dt}=min(\mu\cdot\gEQT\cdot\Ns,\phos_{\Veff}\cdot\NP)-\dmNf (\#eq:Nf)
\end{equation}
```
...
\begin{equation}
    \displaystyle 
    \frac{d\D}{dt}=\frac{\flow\cdot{}dt}{\xfarm}\cdot(\D_{ext}-\D)+ (\dmNf- \rlD)\cdot\frac{\VMA}{\Vint} (\#eq:D)
\end{equation}
```
...
algaeparams<-data.frame(parameter=nicenames,description=descriptions,units=units,ulva=as.character(as.numeric(parms_ulva))[1:19],saccharina=as.character(as.numeric(parms_saccharina))[1:19],alaria=as.character(as.numeric(parms_alaria))[1:19])
...
The following parameter values are needed for post processing to convert model outputs into the quantities required by the funder:

1. Macroalgal C:N - converts net N uptake into carbon for the purposes of CO2 balance
2. energy content of macroalgae - required to convert seaweed biomass into nutritional energy content (to give kcal/m$^{-2}$)
3. protein content as a fraction of dry weight to give protein production in kg/m$^{-2}$

Values are taken from literature as follows:



C:N Saccharina varied from 18 (11m depth) to 48 (4m depth) in @Nielsen2014 in Aarhus bay, Denmark. @Gevaert2001 find C:N form 7.1 (March) to 12.8 (September) in the English Channel. More modest values of 6 to 8 are found by @Monteiro2020 off the Brittany coast but these are mass C:N not molar - so convert to 7 to 9.3. Finally, @Diehl2021 find ratios of between 13 and 26 accross the full latidudinal range of S. Lattisima with highest values found in Helgoland (N. Sea) and higher C:N values attributed to nutrient limitation.. 

C:N Ulva also appears to be related to nutrient availability and highly variable. @Neiori1991 find C:N between 17.5 (low nutrient loading) and 8 (high nutrient) when U. Lactuta was grown as a biofilter in fish farm effluent. @Nielsen2012 find C:N ranging from 7 to 23 in experiments where seaweed was fertilised with manure. @Bruhn2011 found U. lactuta C:N varied from 10 to 25 depending on light irradiance in growth experiments. At 'low' nutrient concentrations (~10uM of NH4 so actually quite high compared to the natural environment) in high-through-flow experiments, C:N of U. Lactuta was found to be between 8 and 10. 

There is little available data for Alaria spp. @Buchholz2015 found C:N in Alaria biomass varying between 30 (healthy) and 15 (dying) in Svalbard. 

In a comprehensive review of macromolecular composition of wild populations of non-calcifying macroalgae, @Fiset2019 find median C:N ratios of ~15 (range 13-18) for green algae species (chlorophyta, including ulva spp) and ~21 (18-24) for ochrophyta (brown algae, including Saccharina and Alaria). These phylum level values are broadly consistent with the species or genus-specific data summarised above - particularly for populations of macroalgae growing at normal seawater concentrations, which would be low relative to incubation experiments or fish farm outflow etc. Therefore we adopt the phylum-averages of 15 for ulva and 21 for Saccharina and Alaria. 



The same review provides the necessary information to derive representative energy and protein values per g DW for each phylum. Table below shows summary values from Fig 1 of @Fiset2019.


```{r nutritionalcomposition, echo=FALSE}
  

  composition<-data.frame(
    Phylum=c('Rhodophyta','Ochrophyta','Chlorophyta'),
    Protein=c(10.5,8,10.5),
    Lipid=c(2.2,2.9,3.2),
    Carbohydrate=c(48,52,46)
  )
   knitr::kable(modelouts,caption = 'Nutritional composition of seaweeds from Fiset et al., (2019). Note that only Brown (Ochrophyta) seaweeds (Saccharina, Alaria) and Green (Chlorophyta) seaweeds (Ulva) are currently parameterised in to the macroalgal model but the Red seaeweed data is included here for future reference.') 
 
 
```


Standard food energy factors ('Atwater factors', @Merrill1973) are used with the above data to calculate the energy composition of seaweeds. Protein (2.44 kcal/g), Fat (lipid, 8.37 kcal/g) and Carbohydrate (3.57 kcal/g). The energy per gram for Ochrophyta is therefore 0.08*2.44 + 0.029*8.37 + 0.52*3.57 = 2.29 kcal / g and similarly for Chlorophyta 2.16 kcal/g
...
b<-  ggplot(refruns) +
 ...
 theme_minimal()
   e<-  ggplot(refruns) +
 aes(x = time, y = kcal_PUA, colour = run) +
 geom_point(shape = "circle", size = 0.7) +
 scale_color_hue(direction = 1) +
 theme_minimal()  
 f<-   ggplot(refruns) +
 aes(x = time, y = Farm_CO2_demand, colour = run) +
 geom_point(shape = "circle", size = 0.7) +
 scale_color_hue(direction = 1) +
 theme_minimal()
...
The growth-yield model runs presented above rely on time-varying environmental forcings to drive growth through a seasonal cycle. This mode of operation allows the modelling of harvest yield for specific grid squares, however the computing resource required to solve the model (order of 2-3 seconds) means that it is not tractable to solve the model for 3 species and >10^5 grid cells in the 'carrying capacity' scenario (potential amount of seaweed that could be grown in every grid square independent of farms in other squares) required by the funder. 

Carrying capacity in population ecology is typically evaluated by considering the steady state solution to a model describing population dynamics in response to resource use. We can take this approach by solving the macroalgal model described above for steady state using the `rootSolve` package. The model is solved once for each month using averaged environmental focings. Solving for steady state 12 times is many times faster than running the model for 24 months in time-varying model. Figure \@ref(fig:bantrysteadystate) shows the biomass (per linear metre) predicted by a steady state run of the model using the Bantry Bay input data. 

```{r bantrysteadystate, echo=FALSE, fig.cap=paste('Bantry bay steady state run showing carrying capacity of biomass in kg per linear metre for a farm of standard density of',default_parms_farm['density_MA'])}
